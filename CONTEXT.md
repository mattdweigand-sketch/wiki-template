<!-- wiki-setup:context-identity:start -->
# <Organization> Wiki - Task Router
<!-- wiki-setup:context-identity:end -->
...
Works with any agent. <!-- wiki-setup:context-status-routing:start -->Start with `AGENTS.md` and check `wiki/domain.md`. An unconfigured clone routes to `SETUP.md` and stops there. A configured wiki reads this file, then opens the selected workspace `CONTEXT.md`.<!-- wiki-setup:context-status-routing:end --> Wrapper details live in `AGENTS.md`; nothing here depends on a wrapper surface.
