...
4. Write one dated entry to `tmp/<run>/log-entry.md`, then follow the [routine finish](../../REFERENCES.md#routine-finalization) with `python3 scripts/finalize_wiki_update.py --log-entry tmp/<run>/log-entry.md`. The shared writer serializes the log and makes an exact retry a no-op. Review the resulting lint output before reporting completion.
