...
5. Adjust `confidence:` only when the realized outcome justifies it. A prediction that converted can move up; one that missed can move down or become `contested` with the reason logged in `wiki/contradictions.md`. This outcome-driven confidence change is routine review, not synthesis promotion.
...
7. Write a dated `review` entry to `tmp/<run>/log-entry.md`, naming pages graded and confidence changes. Run `python3 scripts/finalize_wiki_update.py --log-entry tmp/<run>/log-entry.md` as the [routine finish](../../REFERENCES.md#routine-finalization). It rebuilds backlinks, records the entry, and runs full lint once. Retry the same entry on failure.
