---
name: wiki-setup
description: Ask the initial domain questions, then offer optional GitHub and private-backup connection help.
---
...
This workflow configures the wiki's subject. Domain edits do not configure Git, remotes, dependencies, raw buckets, entity folders, integrations, or infrastructure. After domain setup, it may offer an optional handoff to [`connect.md`](connect.md). That handoff has its own authority checks and is never required to use the wiki.

## Load / Skip

- **Load:** `wiki/domain.md`. Load `connect.md` only after the user accepts a connection handoff.
...
7. Ask whether the user wants help connecting this clone to their own GitHub repository. If yes, load `connect.md` and follow only its GitHub section.
8. Ask whether the user wants to configure a private off-device backup. If yes, load `connect.md` and follow only its backup section.
9. A declined or deferred connection must not block successful domain setup. Do not store repository URLs, backup targets, tokens, or credentials in `wiki/domain.md` or another tracked file.
